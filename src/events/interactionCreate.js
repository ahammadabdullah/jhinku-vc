  ...
  try {
    ...
    if (id === "privacy") {
      if (active.has(userId)) {
        await interaction
          .reply({
            content: t("interaction_already_active", lang),
            flags: MessageFlags.Ephemeral,
          })
          .catch(() => {});
      ...
      collector.on("end", async (_, reason) => {
        ...
        if (reason === "time" && !interaction._wasActiveError) {
          await interaction
            .editReply({
              content: t("interaction_timeout", lang),
              components: [],
            })
            .catch(() => {});
        }
    ...
    if (isStringMenu) {
      const handler =
        modals.get(id) ||
        [...modals.values()].find((m) => m.aliases?.includes(id));
      if (handler) return handler.execute(interaction, client, config);
    }
